...
# once exhausted, new colors are generated procedurally (see _generate_contrasting_color).
PALETTE = [
    (239, 154, 102),  # orange
    (100, 181, 246),  # blue
...
DEFAULT_STORE_PATH = Path(__file__).resolve().parent / "vlan_color_map.yaml"

MIN_CONTRAST_DISTANCE = 140  # good-enough Euclidean RGB distance between distinct VLAN colors
...
def _generate_contrasting_color(existing_colors, attempts=300):
    """Procedurally pick a new color that's as visually distinct as possible
    from every color already in use, by sampling hues spread via the golden
    angle (avoids clustering) and keeping the best-contrast candidate."""
...
def build_vlan_color_map(vlan_ids, store_path=DEFAULT_STORE_PATH):
    """Assign a color to each unique VLAN id seen, plus the fixed 'trunk' and
    'stack' entries, reusing colors from vlan_color_map.yaml when already
    seen in a previous run, and persisting any newly-assigned colors back to
    that file. This keeps VLAN 1, Trunk and Stack colors stable across runs
    too, not just numbered VLANs.
    """
